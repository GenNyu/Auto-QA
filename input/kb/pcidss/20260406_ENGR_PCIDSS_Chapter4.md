...
Figure 1 explains PCI DSS scoping by grouping systems into: the Cardholder Data Environment (CDE), connected-to or security‑impacting systems, and out‑of‑scope systems.

1) CDE (In Scope for PCI DSS)
- System component stores, processes, or transmits CHD/SAD.
- And/or system component is on the same network as, or has unrestricted connectivity to, systems that store, process, or transmit CHD/SAD.

2) Connected-to or Security‑Impacting Systems (In Scope for PCI DSS)
- System component directly connects to the CDE.
- Or system component indirectly connects to the CDE.
- Or system component impacts the configuration or security of the CDE.
- Or system component provides security to the CDE.
- Or system component segments CDE systems from out‑of‑scope systems and networks.
- Or system component supports PCI DSS requirements.

Note: Systems are considered to directly or indirectly connect to the CDE if they can impact the security of the CDE if compromised. For systems that do not directly or indirectly connect to the CDE, controls must be specifically implemented and verified (for example, via penetration testing) to confirm connections to the CDE are not possible.

3) Out‑of‑Scope Systems
- System component does NOT store, process, or transmit CHD/SAD.
- And system component is NOT in the same subnet or VLAN as systems that store, process, or transmit CHD/SAD.
- And system component cannot connect to any system in the CDE.
- And system component does NOT meet any criteria described for connected-to or security‑impacting systems.

If all criteria in category 3 are met, the systems may be considered out of scope for PCI DSS. This may include systems that connect to a connected‑to or security‑impacting system component where controls are in place to prevent the out‑of‑scope system from gaining access to the CDE via the in‑scope system component.
...
KB summary (Figure 2):
- Rule: To reduce PCI DSS scope via segmentation, all in‑scope system components must be segmented from out‑of‑scope systems so that out‑of‑scope systems cannot impact the security of any in‑scope components.
- Decision 1: Is segmentation in place? If no, the entire environment is in scope for the PCI DSS assessment. If yes, the assessor must validate effectiveness of segmentation.
- Decision 2: Did the assessor validate effectiveness? If no, the entire environment is in scope for the PCI DSS assessment. If yes, the assessor documents segmentation is in place and effective, and PCI DSS scope can be limited to areas that store, process, and transmit CHD; are connected to the CDE; and may impact the security of the CDE.
- Evidence: Assessors must document the segmentation used and how its effectiveness was validated.
